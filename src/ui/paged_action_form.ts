...
export class PagedActionFormEvent {
  constructor(
    ...
    pages: Pages,
    pageId?: string,
    options?: PagedActionFormOptions,
    ctx?: any,
  ) {
    this.player = player;
    this.pages = pages;
  ...
  static withPage(
    event: PagedActionFormEvent,
    pageId?: string,
  ): PagedActionFormEvent {
    return new PagedActionFormEvent(
      event.player,
      event.pages,
      pageId,
      event.options,
...
export class PagedActionForm {
  ...
  validatePage(pageId: string, ignorePages: string[] = []): void {
    ...
    ValidationError.optionalValueError(errors, `${pageId}.title`, page.title, [
      "string",
    ]);
    ValidationError.optionalValueError(errors, `${pageId}.body`, page.body, [
      "string",
    ]);
    ValidationError.optionalValueError(errors, `${pageId}.icon`, page.icon, [
      "string",
    ]);
    ValidationError.optionalValueError(
      errors,
      `${pageId}.hidden`,
      page.hidden,
      ["boolean"],
    ...
    if (page.buttons) {
      for (let i = 0; i < page.buttons.length; i++) {
        ...
        const bool = ValidationError.valueError(
          errors,
          `${pageId}.buttons[${i}]`,
          button,
          ["string", "object"],
        ...
        if (typeof button === "object") {
          ValidationError.valueError(
            errors,
            `${pageId}.buttons[${i}].label`,
            button.label,
            ["string"],
          );
          ValidationError.optionalValueError(
            errors,
            `${pageId}.buttons[${i}].icon`,
            button.icon,
            ["string"],
          );
          ValidationError.optionalValueError(
            errors,
            `${pageId}.buttons[${i}].pageId`,
            button.pageId,
            ["string"],
          );
          ValidationError.optionalValueError(
            errors,
            `${pageId}.buttons[${i}].top_margin`,
            button.top_margin,
            ["number"],
          );
          ValidationError.optionalValueError(
            errors,
            `${pageId}.buttons[${i}].bottom_margin`,
            button.bottom_margin,
            ["number"],
          );
          ValidationError.optionalValueError(
            errors,
            `${pageId}.buttons[${i}].top_divider`,
            button.top_divider,
            ["boolean"],
          );
          ValidationError.optionalValueError(
            errors,
            `${pageId}.buttons[${i}].bottom_divider`,
            button.bottom_divider,
            ["boolean"],
          );
          ValidationError.optionalValueError(
            errors,
            `${pageId}.buttons[${i}].onClick`,
            button.onClick,
            ["function"],
          );
          ValidationError.optionalValueError(
            errors,
            `${pageId}.buttons[${i}].condition`,
            button.condition,
            ["function"],
          );
  ...
  show(
    ...
    ctx.handler = this;
    const event = new PagedActionFormEvent(
      player,
      this.pages,
      pageId,
      options,
    ...
    if (this.customPages.has(event.pageId))
      return this.customPages.get(event.pageId)?.show(event);
  ...
  showAll(pageId?: string, options?: PagedActionFormOptions, ctx?: any): void {
    world
      .getAllPlayers()
      .forEach((player) => this.show(player, pageId, options, ctx));
  }
}
